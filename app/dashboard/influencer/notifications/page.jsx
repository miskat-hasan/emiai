...
export default function NotificationsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Heading */}
      <div>
        <h1 className="text-2xl font-bold text-[#203430]">Notifications</h1>
      ...
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/60 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm font-semibold text-primary hover:underline disabled:opacity-40 disabled:no-underline transition-all"
          >
            Make as view
            <Check size={14} />
          </button>
          ...
          {notifications.map(n => (
            ...
            >
              ...
              <div
                ...
                ${n.read ? "bg-gray-100 text-[#63716E]" : "bg-primary/10 text-primary"}
              `}
              >
                <Bell size={16} />
              </div>
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`text-sm font-semibold leading-tight mb-1 ${n.read ? "text-[#63716E]" : "text-[#203430]"}`}
                >
                  {n.title}
                  {!n.read && (
                ...
                <p className="text-xs text-[#63716E] leading-relaxed line-clamp-2">
                  {n.body}
                </p>
              </div>

              {/* Time */}
              <span className="text-xs text-[#63716E] shrink-0 mt-0.5 whitespace-nowrap">
                {n.time}
              </span>
            </div>
          ))}
