// components/dashboard/inbox/InboxSidebar.jsx
"use client";
...
const InboxSidebar = React.memo(
  ({ activeTab, setActiveTab, counts, onCreateGroup }) => {
    const tabs = [
    ...
    return (
      <div className="w-full lg:w-[250px] shrink-0 flex flex-col gap-6">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-primary to-secondary text-white py-3.5 rounded-xl font-medium shadow-[0_8px_20px_theme(colors.primary/25%)] hover:opacity-90 transition-opacity cursor-pointer"
        >
          <Plus size={20} />
          Create Group
        </button>
          ...
          {tabs.map(tab => {
            ...
            return (
              <button
                ...
                className={`
                w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-all duration-200 cursor-pointer
                ${
                  isActive
